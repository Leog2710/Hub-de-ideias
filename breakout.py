...
def game_loop():
    paddle = pygame.Rect(
        SCREEN_WIDTH//2 - PADDLE_WIDTH//2,  # x: centro horizontal
        SCREEN_HEIGHT - 50,                 # y: próximo à parte inferior
        PADDLE_WIDTH,
        PADDLE_HEIGHT
    )
    ...
    speed = 4  # velocidade inicial
    angle = random.uniform(-math.pi/4, math.pi/4)  # ângulo entre -45° e 45°
    direction = random.choice([-1, 1])  # decide se vai para a esquerda ou direita
    ball_speed = [speed * math.cos(angle) * direction, -speed * math.sin(angle)]
    ...
    while running:
        ...
        # Fim de jogo se a bola sair pela parte inferior
